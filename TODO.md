...
are accepted; spec revision 1.16 records the §56 shaping decision. This pass does
**not** mark the entire backlog complete.
...
rows below remain the per-feature source of truth; checked rows are not reopened merely
because a broader feature family still has residue.
...
- Fold steering's private interceptTime into prediction's export — interceptTime fold DONE 2026-09-06; ~~spatial-hash neighbors~~ DONE 2026-09-06; ~~spherical wander~~ DONE 2026-09-06; ~~CCD/FABRIK~~ DONE 2026-09-06; ~~path-planning adapters (RFC)~~ **Accepted; first packet implemented 2026-09-11** (`docs/rfcs/0007-path-planning-adapters.md`; corrected 2026-09-10, four-agent plan `docs/plans/RFC-0007-PATH-PLANNING_PLAN.md`); ~~text shaping engine (RFC)~~ **Accepted; first packet implemented 2026-09-11** (`docs/rfcs/0008-text-shaping-engine.md`; corrected 2026-09-10, plan `docs/plans/RFC-0008-TEXT-SHAPING_PLAN.md`); robotic joint commands utility (MAY declined — see prediction.ts staging note)
...
- [ ] **RFC 0001 residue:** std140 WebGL uniform blocks with call/byte
      measurement, reusable named subgraphs, safe data-declared operators and
      per-node GLSL/WGSL source maps **DONE 2026-09-11**. Finite explicit
      `ShaderVariantSet` families also shipped. Still open: IR-level conditional
      variants/define cache specialization, graph storage-buffer bindings (§82),
      and automatic lighting-aware graph inputs. See the dated addendum in
      `docs/rfcs/0001-shader-extension-addendum.md`; explicit variants do not
      silently close the IR conditional requirement.
...
- [ ] **RFC 0003 residue (staged in source, 2026-08-28):** GPU morph path (the
      ...
      ~~CPU skinning~~ **DONE 2026-09-11** (`CpuSkinning` + `Mesh.skinningMode`,
      interpolated geometry, bounds/picking and serialization, same-runtime golden);
      bone-texture palette (unbounds
...
- [ ] Lighting follow-ups (MVP tier shipped 2026-08-04 — see Done):
      ...
      shadows remain),
      §59 StandardMaterial/PBR rest (`normalMap` / `occlusionMap` WebGL DONE 2026-09-11, WebGPU open;
...
- [x] **PH-22 residue (re-read 2026-08-21):** ~~`PH-22f` joint-anchor mutability~~
      ...
      **DONE 2026-09-06** (`solveCCD` / `solveFABRIK` + joint limits + iteration
      budget); ~~path-planning adapters still want an RFC~~ **DONE 2026-09-11**
      (RFC 0007 accepted; waypoint A*, steering and trajectory conversion);
...
- [ ] **R-30c — the rest of §77, scoped by why each is not ordinary work:**
      ...
      ~~video/`ImageBitmap`, async upload~~ **DONE 2026-09-11** (host surface
      adapters + frame invalidation, explicit `prepareTexture` fences/queue completion).
...
- [x] Fold steering's private interceptTime into prediction's export (dated note in
      ...
      **Accepted; first packet implemented 2026-09-11** (`docs/rfcs/0007-path-planning-adapters.md`);
...
- [x] Before §56 full text shaping: RFC the shaping engine (HarfBuzz-wasm vs native)
      — **Accepted; first packet implemented 2026-09-11** (`docs/rfcs/0008-text-shaping-engine.md`; corrected
...
- [x] 2026-09-09 — **Open-TODO wave 4 slices (simple → complex).**
      ...
      `ParticleIdProgram` _(landed later the same day — see RFC 0005
      body)_. Still owner/hardware: first publish, R-33
      exit.
...
- [x] 2026-09-09 — **Open-TODO second-wave slices (simple → complex).**
      ...
      open on those rows: WebGPU `mapAsync` picking _(landed later the
      same day)_, multi-light / shadows / PBR / §60a. Still
...
- [x] 2026-09-09 — **Open-TODO first-wave slices (simple → complex).** Did
      not close any of the 13 checkboxes _(R-19/R-20 closed later that day;
      recount is 12)_. Landed: `Matrix3.setNormalFromMatrix4` + WebGL lit/standard `uniform mat3 normalMatrix`; RFC 0005
...
- [x] 2026-09-06 — **§83 dev warnings (A-5 remainder, #73).** `warnDisposedInUse`,
      ...
      still open _(closed 2026-09-09 — opt-in `auditFinalizedLeaks` is
      the design)_.
...
- [x] 2026-09-06 — **Camera rigs docs (#73).** Trackball + fly snippet; shake open
      _(CameraShake landed later the same day)_.
